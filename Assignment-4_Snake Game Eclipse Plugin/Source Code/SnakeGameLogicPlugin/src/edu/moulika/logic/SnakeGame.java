 ...
 *
 */
public class SnakeGame extends JFrame implements GameInterface {
	 ...
	 */
	public BoardInterface board;// 1.1vc
	 ...
	 */
	private ClockInterface logicTimer; // 1.1vc change Clock to SClock

	public SidePanelInterface side; // 1.1vc

	private IConfigurationElement[] elements; // 1.1vc
	...
	public SnakeGame() {
		super("Snake Game");
		elements = Platform.getExtensionRegistry().getConfigurationElementsFor("edu.moulika.clockExtPoint"); // 1.1vc
		...
		/*** 1.1vc starts added Extension code to handle BoardPanel ****/

		for (IConfigurationElement e : elements) {
			try {
				final Object o = e.createExecutableExtension("BoardPanel");
				if (o instanceof BoardInterface) {
					this.board = (BoardInterface) o;
					board.init(this);
					add((Component) board, BorderLayout.CENTER);
				}
		...
		/*** 1.1vc ends added Extension code to handle BoardPanel ****/
		...
		/*** 1.1vc starts added Extension code to handle SidePanel ****/
		for (IConfigurationElement e : elements) {
			try {
				final Object o = e.createExecutableExtension("SidePanel");
				if (o instanceof SidePanelInterface) {
					this.side = (SidePanelInterface) o;
					side.init(this);
					add((Component) side, BorderLayout.EAST);
				}
			} catch (CoreException e1) {
				// e1.printStackTrace();
			}
		}
		/*** 1.1vc ends added Extension code to handle SidePanel ****/
	...
	public void startGame() {
		...
		/*** 1.1vc starts added Extension code to handle Clock ****/
		for (IConfigurationElement e : elements) {
			try {
				final Object o = e.createExecutableExtension("Clock");
				if (o instanceof ClockInterface) {
					this.logicTimer = (ClockInterface) o;
					this.logicTimer.init(9.0f);
				}
			} catch (CoreException e1) {
		...
		// this.logicTimer = new SClock(9.0f);
		...
		while (true) {
			...
			((Component) board).repaint(); // 1.1vc
			((Component) side).repaint(); // 1.1vc
			/*
	...
	private TileType updateSnake() {
		 ...
		 * identically.
		 */
		if (head.x < 0 || head.x >= BoardInterface.COL_COUNT || head.y < 0 || head.y >= BoardInterface.ROW_COUNT) { // 1.1vc
																													// added
																													// BoardInterface
	...
	private void resetGame() {
		 ...
		 */
		Point head = new Point(BoardInterface.COL_COUNT / 2, BoardInterface.ROW_COUNT / 2);
		// 1.1vc added BoardInterface
	...
	private void spawnFruit() {
		 ...
		 * board.
		 */
		int index = random.nextInt(BoardInterface.COL_COUNT * BoardInterface.ROW_COUNT - snake.size()); // 1.1vc
																										// added
																										// BoardInterface
		 ...
		 */
		int freeFound = -1;
		for (int x = 0; x < BoardInterface.COL_COUNT; x++) { // 1.1vc added
																// BoardInterface
			for (int y = 0; y < BoardInterface.ROW_COUNT; y++) { // 1.1vc added
																	// BoardInterface
